...
# --- DATEN FILTERN ---
# 1. Wir holen uns die aktuelle Zeit in Berlin
now = datetime.now(pytz.timezone('Europe/Berlin'))
df_stats = df.copy()

if zeitraum == "Aktuelle Session":
    # Start von heute (00:00 Uhr) berechnen
    today_start_aware = now.replace(hour=0, minute=0, second=0, microsecond=0)
    
    # Startzeitpunkt (gestern + heute)
    # WICHTIG: .replace(tzinfo=None) entfernt die Zeitzone für den Vergleich mit der Tabelle
